...
#### 4.2.1 Structured Clones

**Andreu Botella** had recently reviewed Structured Clone, building off of Philip Chimento’s [Shadow Realms-related WPT Pull Request](https://github.com/web-platform-tests/wpt/pull/49282), more analysis is needed.

#### 4.2.2 DOM Exception

All DOM Exception tests appear compatible with the Minimum Common API

#### 4.2.3 Event & EventTarget

Events necessarily involve bubbling, which is fully reliant on the DOM. The majority of these tests will not run or pass in a server-side runtime. The committee will return to this topic in the future, with Events (and EventTargets) as their own timeboxed agenda item.

#### 4.2.4 WASM

All but one module serialization test for WASM is appropriate for a server-side runtime and shall be included in MCApi compliance
